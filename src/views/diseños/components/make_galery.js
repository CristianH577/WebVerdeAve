...
import './make_galery.css'

import addLangText from '../../../lang/diseños/components/make_galery.json'
...
function MakeGalery({ model, data }) {
    ...
    const artImgs = require.context('../../../assets/imgs/galeries', true)
    ...
    const DesignDefault = (article) => {
        return <Image
            ...
            src={artImgs(`./galeries (${article.key}).png`)}
            className="w-[360px] h-[360px] object-contain bg-white p-2 max-[360px]:rounded-none"
        />
    }
        ...
        >
            <CardBody className="overflow-visible p-0 ">
                <Image
                    ...
                    alt={article.title || langText.card.title + ' ' + article.key}
                    src={artImgs(`./galeries (${article.key}).png`)}
                    className={"h-[360px] object-contain bg-white p-2 max-[360px]:rounded-none " + (display === 'list' ? 'w-full' : 'w-[360px]')}
                />
            </CardBody>
        ...
        switch (columnKey) {
            case "img":
                cellContent =
                    <Image
                        shadow="sm"
                        className=" h-[360px] object-contain bg-white p-2 max-[360px]:rounded-none"
                        src={artImgs(`./galeries (${item.key}).png`)}
                        alt={item.title}
                        width={'100%'}
                        classNames={{
                            wrapper: 'w-[360px]'
